  ...
  <div class="card card-primary card-outline card-outline-tabs">
    <div class="card-header p-0 border-bottom-0">
      ...
      <?php foreach ($jasa as $row): ?>
        <li class="nav-item mx-auto w-25">
          <a class="nav-link active p-2 w-100 text-center" id="<?= $row['nama_kategori'] ?>-tab" data-toggle="pill" href="#<?= $row['nama_kategori'] ?>" role="tab" aria-controls="<?= $row['nama_kategori'] ?>" aria-selected="true"><?= $row['nama_kategori'] ?></a>
        </li>
      <?php endforeach; ?>
      </ul>
    </div>
    ...
    <?php foreach ($jasa as $row): ?>
      <div class="tab-content" id="custom-tabs-three-tabContent">
        <div class="tab-pane fade show active" id="<?= $row['nama_kategori'] ?>" role="tabpanel" aria-labelledby="<?= $row['nama_kategori'] ?>-tab">
          <?php foreach ($row['jasa_sub_kategori'] as $sub): ?>
            <a href="<?= base_url('jasa/') . $sub['id_kategori_jasa'] . '/' . $sub['id_sub_kategori_jasa'] ?>" class="btn btn-md btn-warning mb-1" style="border-radius:25px"><?= $sub['nama_sub_kategori_jasa'] ?></a>
          <?php endforeach; ?>
        </div>
      </div>
    <?php endforeach; ?>
...
<div class="row bg-white mt-5 shadow">
  <div class="row col-md-10 mx-auto p-3 mb-3">
    ...
    <div class="col-md-6">
      <div class="text-dark text-tawaran mb-3">JASA APA SAJA YANG KAMI TAWARKAN ?</div>

        <?php foreach ($kategori as $row): ?>
        <div class="">
          <h5 class="m-0 text-dark">Jasa <?php echo $row->nama_kategori; ?></h5>
        </div>
        <?php endforeach; ?>

      <div class="col-md-12 mt-3">
        <a href="<?= base_url('jasa') ?>" class="btn text-dark btn-lg btn-warning shadow-sm">
